...
public class Reduction {
	...
	boolean all(Iterable<E> xs, Predicate<? super E> p) {
		return fold(xs, true, (result, x) -> p.test(x));
	}


	public static <E>
	Set<E> collect(Iterable<E> xs, Predicate<? super E> p) {
		return fold(xs, new HashSet<>(), (result, x) -> {
	...
	public static int readBinary(Iterable<Character> xs) {
		return fold(xs, 0, (result, x) -> result * 2 + (x == '1' ? 1 : 0));
	}
	...
	public static Pair<Integer, Integer> readRoman(Iterable<Character> xs) {
		return fold(xs, new Pair<>(0, 0), (result, x) -> (romanDigit(x) > result.snd ?
				new Pair<>(result.fst - 2 * result.snd + romanDigit(x), romanDigit(x))
				: new Pair<>(result.fst + romanDigit(x), romanDigit(x))));
	}
